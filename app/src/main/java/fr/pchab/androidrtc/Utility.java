...
public class Utility {
    ...
    public static void longPressWindowCenter(long maxTime, AccessibilityService.GestureResultCallback cb){
        if(AccService.instance == null){
            return;
        }
        AccessibilityNodeInfo root = AccService.instance.getRootInActiveWindow();
        if(root == null){
            return;
        }
    ...
    public static void longPress(AccessibilityNodeInfo node, long maxTime, AccessibilityService.GestureResultCallback cb){
        ...
        new Thread(){
            @Override
            public void run() {
                AccService.instance.dispatchGesture(builder.build(), cb, null);
            }
        }.start();
